...
for (i in seq(from = 0, to = 12, by = 1)){  #step by 1 mths
    ...
    set.seed(1)
    mass_i <- as_data_frame(rnorm(MC_reps, mass, sd), col.names = str(i))
...
#turn matrix into data frame (need to include the t() to keep the matrix format)
MC_mass_diffs_t <- MC_mass_diffs %>% as_data_frame()
...
mean_masschange <- as_data_frame(mean_masschange)
...
sd_masschange <- as_data_frame(sd_masschange)
...
#turn matrix into data frame (need to include the t() to keep the matrix format)
MC_female_mass_diffs_t <- MC_female_mass_diffs %>% as_data_frame()
...
mean_masschange_female <- as_data_frame(mean_masschange_female)
...
sd_masschange_female <- as_data_frame(sd_masschange_female)
...
#turn matrix into data frame (need to include the t() to keep the matrix format)
MC_male_mass_diffs_t <- MC_male_mass_diffs %>% as_data_frame()
...
mean_masschange_male <- as_data_frame(mean_masschange_male)
...
sd_masschange_male <- as_data_frame(sd_masschange_male)
