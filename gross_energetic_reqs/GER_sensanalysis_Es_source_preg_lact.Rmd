---
title: "Gross Energetic Requirements(GER) Sensitivity Analysis - Preg/Lact"
author: "Selina Agbayani"
date: "09 March 2022 -  code updated `r format(Sys.time(), '%d %B, %Y')`"
output:
  html_document:
...
P_cost_table_preg$Ts <-  396 #total no. of days pregnant
...
#should this be 293 days??? i.e. total no. of days lactating?
...
mean_masschange <- mean_masschange %>% filter(age_yrs >=0)
...
mean_masschange_peryear <- mean_masschange_peryear %>% filter(age_yrs >= 0)
...
for (i in seq(from = 8, to = 75, by = 1)){
  for (MC_var in c("all","Hg","GER_calf", 
   ...
   # Monte Carlo - Production cost 
    ...
    MC_vars_i$GER <- NA
    MC_vars_i<- MC_vars_i %>% relocate(sex, GER) #move sex and GER to the front of column
    ...
    MC_vars_i$GER_foraging <- ((P_cost + Es + Hg + P_nb + GERcalf_0to6m )/153)/E_FnU #days foraging while pregnant = 153
    ...
    #153 days foraging while preg, 134 days foraging while lact)
    MC_vars_i <- MC_vars_i %>% 
...
for (i in seq(from = 8, to = 74, by = 1)){
  for (MC_var in c("all","P_cost","Es","GERcalf","E_FnU")){
    ...
    MC_vars_i$GER <- NA
    MC_vars_i<- MC_vars_i %>%  
      relocate(sex, GER) #move sex and GER to the front columns
