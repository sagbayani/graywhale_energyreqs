...
```{r panel plot Es sensitivity analysis all stages}
...
# New facet label names for MC_var_factor variable
MC_variable.labs <- c("All variables","%O2 absorbed", "Resp. rates", "Tidal Vol.")
names(MC_variable.labs) <- c("all","pctO2", "Rs", "Vt")
...
plot_Es_sensAnalysis_phase1_permth <- Es_sensAnalysis_phase1_permth %>% 
  ...
  facet_grid(Lifestage~MC_variable, 
             labeller = labeller(MC_variable = MC_variable.labs)) +
  xlab("Age (months)") +
...
```{r Es sensanalysis allstages multiplot}
